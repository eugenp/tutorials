...
@Repository public class CoffeeRepositoryImpl implements CoffeeRepository {

        private Map<String, Coffee> CoffeeStore = new HashMap<String, Coffee>();

        @Override public void createCoffee(Coffee Coffee) {
                CoffeeStore.put(Coffee.getName(), Coffee);
        }

        @Override public Coffee getCoffee(String name) {
                return CoffeeStore.get(name);
        }

        @Override public List<Coffee> getAllCoffee() {
