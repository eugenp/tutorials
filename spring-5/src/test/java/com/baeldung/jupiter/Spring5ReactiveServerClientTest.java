...
public class Spring5ReactiveServerClientTest {
    ...
    public static void setUp() throws Exception {
        ...
        RouterFunction<?> route = RouterFunctions
          .route(POST("/task/process"), request -> ServerResponse.ok()
    ...
    private static Flux<Task> getLatLngs() {
        return Flux.range(0, 3).zipWith(Flux.interval(Duration.ofSeconds(1)))
    ...
    public void givenCheckTask_whenServerHandle_thenServerResponseALiveString()
      throws Exception {
        ...
        Mono<String> result = client.get().uri("/task").exchange()
