...
public class HashMapUpdateUnitTest {
    ...
    public void givenFruitMap_whenKeyExists_thenValuesUpdated() {
        ...
        if (fruitMap.containsKey("watermelon")) {
            fruitMap.put("watermelon", newValue);
        }

        Assertions.assertEquals(Double.valueOf(newValue), fruitMap.get("apple"));
        Assertions.assertFalse(fruitMap.containsKey("watermelon"));
    }
    ...
    public void givenFruitMap_whenComputeUsed_thenValueUpdated() {
        ...
        Assertions.assertEquals(
                Double.valueOf(Math.pow(oldPrice, 2)),
                fruitMap.get("apple"));
        Assertions.assertThrows(
                NullPointerException.class,
                () -> fruitMap.compute("blueberry", (k, v) -> powFunction.apply(v, 2)));
