...
import com.baeldung.reactive.util.CpuUtils;
...
public class CpuUsageEventProducer {
    ...
    Flux<CpuUsageEvent> events() {
        Flux<CpuUsageEvent> eventFlux = Flux.fromStream(Stream.generate(() -> new CpuUsageEvent(CpuUtils.getUsage(), LocalDateTime.now())));
