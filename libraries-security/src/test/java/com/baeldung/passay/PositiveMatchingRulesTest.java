...
public class PositiveMatchingRulesTest {
    ...
    public void givenPasswordValidationRules_whenValidatingPassword_thenPosswordIsNotValidWithSeveralErrors() {
        ...
        assertEquals("INSUFFICIENT_LOWERCASE:{" +
                "minimumRequired=5, matchingCharacterCount=3, " +
                "validCharacters=abcdefghijklmnopqrstuvwxyz, " +
                "matchingCharacters=abc}", getDetail(validate, 3));
    ...
    public void givenRulesForDifferentPasswordLength_whenValidatingTwoDifferentPassword_thenBothOfThemAreInvalid() {
        ...
        shortPassword.setPassword("12ab");

        PasswordData longPassword = new PasswordData();
        longPassword.setPassword("1234abcde");
        ...
        assertEquals("INSUFFICIENT_LOWERCASE:{" +
                ...
                "validCharacters=abcdefghijklmnopqrstuvwxyz, " +
                "matchingCharacters=ab}", getDetail(validateShort, 0));
    ...
    public void givenCharacterCharacteristicsRule_whenValidatingPassword_thenItIsInvalidAsItBreaksToManyRules() {
        ...
        assertEquals("INSUFFICIENT_UPPERCASE:{" +
                "minimumRequired=5, " +
                "matchingCharacterCount=0, " +
                "validCharacters=ABCDEFGHIJKLMNOPQRSTUVWXYZ, " +
                "matchingCharacters=}", getDetail(validate, 0));
        assertEquals("INSUFFICIENT_CHARACTERISTICS:{" +
                "successCount=3, " +
                "minimumRequired=4, " +
                "ruleCount=4}", getDetail(validate, 1));
    }
