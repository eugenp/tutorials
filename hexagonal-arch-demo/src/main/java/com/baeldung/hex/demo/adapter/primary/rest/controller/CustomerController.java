...
@RestController @RequestMapping("/customer") public class CustomerController {
        ...
        @GetMapping("/{customerId}") public ResponseEntity <CustomerResource> getCustomerById(@PathVariable String customerId) {
                return ResponseEntity.ok().body(customerAdapter.toResource(customerService.getCustomer(customerId)));
        }

        @PutMapping public ResponseEntity <CustomerResource> updateCustomerAddress(Customer customer) {
                return ResponseEntity.ok().body(customerAdapter.toResource(customerService.updateCustomerAddress(customer)));
        }

        @PostMapping public ResponseEntity <CustomerResource> createCustomer(CustomerResource customerResource) {
        ...
        @GetMapping public ResponseEntity <Boolean> isCustomerEligibleForUpgrade(@PathVariable String customerId) {
