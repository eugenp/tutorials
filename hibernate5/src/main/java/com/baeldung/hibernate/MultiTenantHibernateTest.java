...
public class MultiTenantHibernateTest {
	...
	private void initDb1(Session db1Session) {
		...
		db1Session
				.createSQLQuery(
						"create table Suppliers (id integer generated by default as identity, country varchar(255), name varchar(255), primary key (id))")
				.executeUpdate();
		db1Session.createSQLQuery("insert into Suppliers (id, country, name) values (null, 'John', 'USA')")
				.executeUpdate();
		transaction.commit();
	}
	...
	private void initDb2(Session db2Session) {
		...
		db2Session
				.createSQLQuery(
						"create table Suppliers (id integer generated by default as identity, country varchar(255), name varchar(255), primary key (id))")
				.executeUpdate();
		db2Session.createSQLQuery("insert into Suppliers (id, country, name) values (null, 'Miller', 'UK')")
				.executeUpdate();
		transaction.commit();
	}
}
