...
public class TweetControllerTests {
    ...
    @Test
    public void getDefaultTweet() throws Exception {
    	ResponseEntity<Tweet> response = getTweetById("1");
    	assertEquals(OK, response.getStatusCode());
        assertEquals("default tweet", response.getBody().getText());
    }

    @Test
    public void addTweet() throws Exception {
    	Tweet tweet = Tweet.createTweet("2",  "My first sample tweet");
        ResponseEntity<Void> addTweetResponse = restTemplate.postForEntity("/tweets/addTweets",tweet, Void.class);
        ...
        assertEquals("2", getTweetResponse.getBody().getId());
        assertEquals("My first sample tweet", getTweetResponse.getBody().getText());
    }

    @Test
    public void addTweet_Without_Id_Fails() throws Exception {
    ...
    @Test
    public void addTweet_Without_Text_Fails() throws Exception {
    	Tweet tweet = Tweet.createTweet("3",  "");
        assertUnProcessedEntity(tweet);

        ResponseEntity<Tweet> getTweetResponse = getTweetById("3");
    	assertEquals(OK, getTweetResponse.getStatusCode());
    	assertNotEquals("3", getTweetResponse.getBody().getId());
    	assertNotEquals("", getTweetResponse.getBody().getText());
    }
