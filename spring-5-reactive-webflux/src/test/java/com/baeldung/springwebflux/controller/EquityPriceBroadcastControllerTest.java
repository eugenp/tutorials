...
public class EquityPriceBroadcastControllerTest {
    ...
    public void testBroadcastEquityPrices() {
        ...
        FluxExchangeResult<Equity> result = this.testClient.get().uri("/api/getEquityPrice")
                .accept(MediaType.APPLICATION_STREAM_JSON)
                .exchange()
                .expectStatus().isOk().returnResult(Equity.class);

        Flux<Equity> resultBody = result.getResponseBody();
        StepVerifier.create(resultBody).consumeNextWith(equity -> {
