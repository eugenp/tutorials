...
class SecondLevelCacheIntegrationTest {
    ...
    @Test
    final void givenEntityIsLoaded_thenItIsCached() {
    ...
    @Test
    final void givenBarIsUpdatedInNativeQuery_thenFoosAreNotEvicted() {
        final Foo foo = new Foo(randomAlphabetic(6));
        fooService.create(foo);
        fooService.findOne(foo.getId());
        ...
        new TransactionTemplate(platformTransactionManager).execute(status -> {
            ...
            nativeQuery.unwrap(Query.class);
    ...
    @Test
    final void givenCacheableQueryIsExecuted_thenItIsCached() {
        new TransactionTemplate(platformTransactionManager).execute(status -> {
