(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('baeldungApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-baeldungApp-error');
                var entityKey = httpResponse.headers('X-baeldungApp-params');
                if (errorHeader) {
