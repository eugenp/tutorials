...
public class WebfluxSecurityIncidentApplicationTest {
	...
	@Test
	public void testGetSecurityEvents() throws InterruptedException  {
		webTestClient
		        .get()
				...
				.consumeWith(securityEvents -> {
					...
					assertThat(securityEvents.getResponseBody())
						.allSatisfy(securityEvent -> assertThat(securityEvent.getId().toString().length()).isEqualTo(UUID_LENGTH))
						.allSatisfy(securityEvent -> assertThat(securityEvent.getName().length()).isPositive())
						.allSatisfy(securityEvent -> assertThat(securityEvent.getTimestamp()).isInThePast());
				});
	}

	@Test 
	public void testGetSecurityEventStream () throws InterruptedException {
