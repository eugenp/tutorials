...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ng="http://docbook.org/docbook-ng"
                ...
                extension-element-prefixes="exslt" version="1.0" exclude-result-prefixes="exsl db ng exslt">
        ...
        <xsl:variable name="href">
            <xsl:call-template name="href.target.with.base.dir">
                <xsl:with-param name="context"
                                select="/"/>        <!-- Generate links relative to the location of root file/toc.xml file -->
            </xsl:call-template>
        </xsl:variable>
            ...
            <xsl:choose>
                <xsl:when test="$rootid != ''">
                    <xsl:apply-templates select="key('id',$rootid)//indexterm" mode="idx">
                        <xsl:sort
                                select="normalize-space(concat(primary/@sortas, primary[not(@sortas) or @sortas = '']))"/>
                        <xsl:sort
                                select="normalize-space(concat(secondary/@sortas, secondary[not(@sortas) or @sortas = '']))"/>
                        <xsl:sort
                                select="normalize-space(concat(tertiary/@sortas, tertiary[not(@sortas) or @sortas = '']))"/>
                    </xsl:apply-templates>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="indexterm|primary|secondary|tertiary" mode="idx">
        ...
        <xsl:variable name="text">
            ...
            <xsl:if test="following-sibling::*[1][self::see]">
                ...
                <xsl:text> </xsl:text>
                <xsl:value-of select="following-sibling::*[1][self::see]"/>)
            </xsl:if>
        </xsl:variable>
