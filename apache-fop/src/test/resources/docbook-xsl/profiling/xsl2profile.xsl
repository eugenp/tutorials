...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                <xsl:choose>
                    <xsl:when test="local-name(.) = 'extension-element-prefixes' or
                        ...
                        <xsl:attribute name="{local-name(.)}">
                            <xsl:value-of select="concat(., ' exslt')"/>
                        </xsl:attribute>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:attribute name="{local-name(.)}">
    ...
    <xsl:template
            match="xsl:template[@name='hhc-main' or @name='hhp-main' or @name='hhk' or @name='hh-map' or @name='hh-alias' or @name='etoc'] | xsl:variable[@name='raw.help.title']">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates mode="correct"/>
        </xsl:copy>
            ...
            <xsl:for-each select="@*">
                <xsl:choose>
                    ...
                    <xsl:when test="local-name(.) = 'select' and starts-with(., '/')">
                        <xsl:attribute name="{local-name(.)}">$profiled-nodes<xsl:value-of select="."/>
                        </xsl:attribute>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:attribute name="{local-name(.)}">
                    ...
                    <xsl:when test='contains(., "key(&apos;id&apos;,$rootid)")'>
                        <xsl:attribute name="{local-name(.)}">
                            <xsl:call-template name="string.subst">
                                ...
                                <xsl:with-param name="replacement">$profiled-nodes//*[@id=$rootid or @xml:id=$rootid]
                                </xsl:with-param>
                            </xsl:call-template>
                        </xsl:attribute>
                    </xsl:when>
