...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i="urn:cz-kosek:functions:index"
                xmlns:l="http://docbook.sourceforge.net/xmlns/l10n/1.0" xmlns:func="http://exslt.org/functions"
                ...
                extension-element-prefixes="func exslt" exclude-result-prefixes="func exslt i l k" version="1.0">
    ...
    <xsl:template name="generate-kosek-index">
      ...
      <xsl:text>ERROR: the 'kosek' index method requires the
        ...
        <xsl:variable name="terms"
                      select="//indexterm[count(.|key('group-code', i:group-index(normalize-space(concat(primary/@sortas, &quot; &quot;, primary))))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))][1]) = 1 and not(@class = 'endofrange')]"/>
        ...
        <div class="index">
            <xsl:apply-templates select="$terms" mode="index-div-kosek">
                ...
                <xsl:sort select="i:group-index(normalize-space(concat(primary/@sortas, &quot; &quot;, primary)))"
                          data-type="number"/>
            </xsl:apply-templates>
        </div>
    </xsl:template>
    ...
    <xsl:template match="indexterm" mode="index-div-kosek">
        ...
        <xsl:variable name="key"
                      select="i:group-index(normalize-space(concat(primary/@sortas, &quot; &quot;, primary)))"/>
