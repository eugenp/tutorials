        ...
        <xsl:choose>
            <xsl:when test="($target[self::dbs:foil] or $target[self::dbs:foilgroup])">
                ...
                <xsl:variable name="target.no"
                              select="count($target/preceding::dbs:foil|$target/preceding::dbs:foilgroup) + 1 + $generate.titlepage"/>
    ...
    <xsl:template match="db:biblioentry" mode="xref-to">
        ...
        <a>
            <xsl:attribute name="href">
                <xsl:value-of
                        select="concat('#(', count($entry/preceding::dbs:foil|$entry/preceding::dbs:foilgroup) + 1 + $generate.titlepage, ')')"/>
            </xsl:attribute>
            <xsl:choose>
