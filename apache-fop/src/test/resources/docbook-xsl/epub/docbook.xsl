...
<xsl:stylesheet
    ...
    <xsl:param name="epub.ncx.depth">4
    </xsl:param> <!-- Not functional until http://code.google.com/p/epubcheck/issues/detail?id=70 is resolved -->
    ...
    <xsl:template name="package-identifier">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="/*/*[contains(name(.), 'info')]/invpartnumber">
                        <xsl:value-of select="/*/*[contains(name(.), 'info')][1]/invpartnumber[1]"/>
                    </xsl:when>
                    <xsl:when test="/*/*[contains(name(.), 'info')]/issuenum">
                        <xsl:value-of select="/*/*[contains(name(.), 'info')][1]/issuenum[1]"/>
                    </xsl:when>
                    <xsl:when test="/*/*[contains(name(.), 'info')]/productnumber">
                        <xsl:value-of select="/*/*[contains(name(.), 'info')][1]/productnumber[1]"/>
                    </xsl:when>
                    <xsl:when test="/*/*[contains(name(.), 'info')]/seriesvolnums">
                        <xsl:value-of select="/*/*[contains(name(.), 'info')][1]/seriesvolnums[1]"/>
                    </xsl:when>
                    <xsl:when test="/*/*[contains(name(.), 'info')]/volumenum">
                        <xsl:value-of select="/*/*[contains(name(.), 'info')][1]/volumenum[1]"/>
                    </xsl:when>
                    <!-- Deprecated -->
                    <xsl:when test="/*/*[contains(name(.), 'info')]/pubsnumber">
                        <xsl:value-of select="/*/*[contains(name(.), 'info')][1]/pubsnumber[1]"/>
                    </xsl:when>
                </xsl:choose>
                <xsl:text>_</xsl:text>
                <xsl:choose>
    ...
    <xsl:template match="book|
        ...
        <xsl:element name="navPoint" namespace="http://www.daisy.org/z3986/2005/ncx/">
            ...
            <xsl:apply-templates
                    select="book[parent::set]|part|reference|preface|chapter|bibliography|appendix|article|glossary|section|sect1|sect2|sect3|sect4|sect5|refentry|colophon|bibliodiv[title]|setindex|index"
                    mode="ncx"/>
        </xsl:element>
                ...
                <xsl:element namespace="http://www.idpf.org/2007/opf" name="item">
                    <xsl:attribute name="id">
                        <xsl:choose>
                            ...
                            <xsl:when
                                    test="ancestor::mediaobject[@role='cover'] and (count(ancestor::mediaobject//imageobject) = 1)">
                                <xsl:value-of select="$epub.cover.image.id"/>
                            </xsl:when>
