<xsl:stylesheet version="1.0"
    ...
    <xsl:template
            match='*[contains(local-name(), "info")]/*[not(self::title|self::subtitle|self::titleabbrev|self::doc:title|self::doc:subtitle|self::doc:titleabbrev)]'
            priority='0'
            mode='doc:body'/>
    ...
    <xsl:template match='address[not(parent::author|parent::editor|parent::othercredit)] |
                  ...
                  mode='doc:body'>
        <xsl:apply-templates
                select='node()[not(self::affiliation|self::authorblurb|self::doc:affiliation|self::doc:authorblurb)]'/>
    </xsl:template>
    ...
    <xsl:template match='caption|doc:caption'
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:for-each
                        select='text()|*[not(self::para|self::doc:para)]|*[self::para|self::doc:para][position() != 1]'>
                    <xsl:call-template name='doc:nomatch'/>
                </xsl:for-each>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match='entry|doc:entry' mode='doc:body'>
        ...
        <xsl:variable name='width.raw'>
            <xsl:choose>
                    ...
                    <xsl:variable name='combinedWidth'>
                        <xsl:call-template name='doc:sum'>
                            <xsl:with-param name='nodes'
                                            select='ancestor::*[self::table|self::doc:table|self::informaltable|self::doc:informaltable][1]/*[self::tgroup|self::doc:tgroup]/*[self::colspec|self::doc:colspec][not(position() &lt; $position) and position() &lt; $limit]'/>
                ...
                <xsl:otherwise>
                    <xsl:value-of
                            select='ancestor::*[self::table|self::doc:table|self::informaltable|self::doc:informaltable][1]/*[self::tgroup|self::doc:tgroup]/*[self::colspec|self::doc:colspec][position() = $position]/@colwidth'/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template
            match='*[self::para|self::simpara|self::doc:para|self::doc:simpara]/text()[string-length(normalize-space(.)) != 0]'
            mode='doc:body'>
        <xsl:call-template name='doc:handle-linebreaks'/>
    </xsl:template>

    <xsl:template
            match='text()[not(parent::para|parent::simpara|parent::literallayout|parent::programlisting | parent::doc:para|parent::doc:simpara|parent::doc:literallayout|parent::doc:programlisting)][string-length(normalize-space(.)) != 0]'
            mode='doc:body'>
        <xsl:call-template name='doc:handle-linebreaks'/>
    </xsl:template>
    ...
    <xsl:template match='blockquote|doc:blockquote'
        ...
        <xsl:apply-templates
                select='*[not(self::blockinfo|self::title|self::attribution|self::doc:info|self::doc:title|self::doc:attribution)]'
                mode='doc:body'>
