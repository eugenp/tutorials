...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                xmlns="http://www.w3.org/1999/xhtml" exclude-result-prefixes="k" version="1.0">
...
Install those </xsl:text>
        ...
        <xsl:variable name="terms"
                      select="//indexterm[count(.|key('k-group', k:getIndexGroupKey(concat(/*/@lang, /*/@xml:lang), normalize-space(concat(primary/@sortas, &quot; &quot;, primary))))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))][1]) = 1 and not(@class = 'endofrange')]"/>

        <xsl:variable name="alphabetical"
                      select="$terms[not(starts-with(                 k:getIndexGroupKey(concat(/*/@lang, /*/@xml:lang), normalize-space(concat(primary/@sortas, &quot; &quot;, primary))),                 '#NUMERIC'                 ))]"/>

        <xsl:variable name="others"
                      select="$terms[starts-with(                 k:getIndexGroupKey(concat(/*/@lang, /*/@xml:lang), normalize-space(concat(primary/@sortas, &quot; &quot;, primary))),                 '#NUMERIC'                 )]"/>
                    ...
                    <dl>
                        <xsl:apply-templates select="$others" mode="index-symbol-div">
                            ...
                            <xsl:sort lang="{$sort.lang}"
                                      select="k:getIndexGroupSortKey(concat(/*/@lang, /*/@xml:lang),                         k:getIndexGroupKey(concat(/*/@lang, /*/@xml:lang), normalize-space(concat(primary/@sortas, &quot; &quot;, primary))))"/>
                        </xsl:apply-templates>
                    </dl>
                </div>
            </xsl:if>
    ...
    <xsl:template match="indexterm" mode="index-div-kimber">
        ...
        <xsl:variable name="key"
                      select="k:getIndexGroupKey(concat(/*/@lang, /*/@xml:lang), normalize-space(concat(primary/@sortas, &quot; &quot;, primary)))"/>
            ...
            <div class="indexdiv">
                ...
                <dl>
                    <xsl:apply-templates
                            ...
                            mode="index-primary">
                        <xsl:sort select="normalize-space(concat(primary/@sortas, &quot; &quot;, primary))"
                                  lang="{$sort.lang}"/>
                        <xsl:with-param name="scope" select="$scope"/>
                        <xsl:with-param name="role" select="$role"/>
                        <xsl:with-param name="type" select="$type"/>
