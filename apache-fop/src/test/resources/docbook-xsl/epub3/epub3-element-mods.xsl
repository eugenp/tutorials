    ...
    <xsl:template match="book|
        ...
        <xsl:element name="navPoint" namespace="{$ncx.namespace}">
            ...
            <xsl:if test="$depth != 0">
                ...
                <xsl:apply-templates
                        select="book[parent::set]|part|reference|preface|chapter|bibliography|appendix|article|topic|glossary|section|sect1|sect2|sect3|sect4|sect5|refentry|colophon|bibliodiv[title]|setindex|index"
                        mode="ncx"/>
            </xsl:if>
        </xsl:element>
